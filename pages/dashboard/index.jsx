...
import { lStorageSet } from "../../helpers/lStorage";
...
const Dashboard = () => {
  ...
  const getChannelDetails = async () => {
    try {
      setLoading(true);
      const activeChannel = await http_get_request({
        endpoint: `/channel/v1/getActiveChannel/${uid}`,
      });
      setChannel(activeChannel?.results);

      lStorageSet("channel_id", activeChannel?.results?.channel_id);
      lStorageSet("access_key", activeChannel?.results?.access_key);
      lStorageSet("secret_key", activeChannel?.results?.secret_key);
    } catch (err) {
      console.error("Error fetching countries:", err);
      // Handle error if needed
    } finally {
      console.log("activeChannel", "error");
      setLoading(false);
    }
  };
